        ...
        using (System.IO.StreamWriter file = new System.IO.StreamWriter(@"record2.txt", true)) {
            ...
            foreach (var VARIABLE in carRecord) {
                ...
                for (int i = 0; i < VARIABLE.Recordlist.Count; i++) {
                    if (i == 0) {
                        recordS += String.Format(
                            "{0,-10}",
                            Math.Round(VARIABLE.Recordlist[i] * 100f, 2)
                        );
                    } else {
                        recordS += String.Format(
                            "{0,-10}",
                            Math.Round(
                                (VARIABLE.Recordlist[i] - VARIABLE.Recordlist[i - 1]) * 100f,
                                2
                            )
                        );
                    }
                ...
                recordS +=
                    "\r\n"
                    + "加速度为(m/s):"
                    + Math.Round(GameObject.FindWithTag("car").GetComponent<carMove>().getAce(), 1);
                recordS += "\r\n" + "小车质量为(KG):" + Math.Round(VARIABLE.MassesOfcar, 0);
                recordS +=
                    "\r\n"
                    + "拉力为(N):"
                    + Math.Round(VARIABLE.MassesOfWeight * Math.Abs(Physics.gravity.y), 0);
